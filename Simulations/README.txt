This folder consists of the codes for the algorithm used to perform the simulation studies in the supplementary document.
...
1. DataGen_simple.R: the R code used to generate data for Scenario 1 of the simulation study presented in the supplementary document.

   DataGen_complex.R: the R code used to generate data for Scenario 2 of the simulation study presented in the supplementary document.

   DataGen_complex_2.R: the R code used to generate data for Scenario 3 of the simulation study presented in the supplementary document.
...
Regarding the simulation results in the supplementary document, we repeat the following steps for 100 times under each simulation setting by using high performance computing clusters.
...
b. Next issue
   ...
   The number of subfolders depends on the number of folds for the cross validation. In the supplementary document, we use 10-fold cross validation and therefore, there will be 10 subfolders. The number of MATLAB files in each of the 10 subfolders depends on the number of candidate values for the tuning of the first penalty. In my code, I choose five candidate values.
...
c. Execute "sim1.m" , "sim2.m", ..., "sim5.m" in MATLAB/2018b for all the subfolders "fold_1", "fold_2", ..., "fold_10" and save the outputs of "sim1.m" , "sim2.m", ..., "sim5.m" as txt files named "output1.txt", ..., "output5.txt" respectively.
   ...
   Then there should be five txt files named "output1.txt", ..., "output5.txt" (because I used five candidate values) in each of the 10 subfolders "fold_1", "fold_2", ..., "fold_10".
